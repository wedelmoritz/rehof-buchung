...
| **Host-Port-Kollision** | Prod-`web` veröffentlicht den Host-Port **`10.42.42.1:8000`** (`REHOF_PUBLISH_ADDR`). Ein zweiter Stack mit derselben Bind-Adresse kollidiert. | Test-`web` auf **anderen Host-Port** (z. B. `REHOF_PUBLISH_ADDR=127.0.0.1` + anderer Port) per Override (2.2). |
